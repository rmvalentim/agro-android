...
import br.com.rafaelvalentim.agro.Model.Equipamento;
import br.com.rafaelvalentim.agro.WebService.HttpGETService;

public class EquipamentoService {


    public SpinnerAdapter getEquipamentoAdapter(Context context) {
        try {
            // Define a URL que será requisitada no serviço
            URL url = new URL(UrlBaseService.getUrl() + "equipamentos");
            ...
            Gson gson = new Gson();
            TypeToken<List<Equipamento>> token = new TypeToken<List<Equipamento>>() {};
            list = gson.fromJson(retorno, token.getType());

            // Cria um adapter para a ListView da Activity
            SpinnerAdapter adapter = new ArrayAdapter<Equipamento>(context,
